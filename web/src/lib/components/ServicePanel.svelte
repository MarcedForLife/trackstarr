...
{#snippet fileList(items: Row[])}
	<ul class="space-y-2">
		{#each items as file (file.key)}
			<li animate:flip={rowSlide()} in:fade={rowFade()}>
				<RunFile
					run={file.run.id}
					row={file.row}
		...
		>
			...
			{#if waiting}
				<div class="min-w-0">
					...
					<div class="mt-3">
						{@render fileList(shownQueue)}
						{#if queueNotice}<p role="status" class="pt-2 text-[12px] text-dim">
								{queueNotice}
								{#if queueUndo}<button
...
<style>
	/* A run going, said by the dot the heading leads with: the one thing on this
	   page that moves of its own accord. */
	.live {
