...
<button
	...
	class={`flex ${control} ${radius} min-w-0 flex-1 items-center gap-2 border px-3 text-[13px] ${active ? 'border-accent-fill/50 bg-accent-soft text-fg' : 'border-line-strong bg-field text-dim'} hover:text-fg sm:flex-none`}
>
	<Glyph name={icon} size={14} />
	<span class="font-medium">{label}</span>
	{#if detail}<span class="max-w-32 truncate text-dim max-sm:hidden">{detail}</span>{/if}
	{#if active}<span class="h-1.5 w-1.5 flex-none rounded-full bg-accent-fill" aria-hidden="true"
		></span>{/if}
	<span class={`ml-auto sm:ml-0 ${menu.open ? '-rotate-90' : 'rotate-90'}`}
...
<div
	...
	class="fixed m-0 max-h-[calc(100dvh-1.5rem)] w-80 max-w-[calc(100vw-1.5rem)] overflow-y-auto rounded-2xl border border-line-strong bg-raised/85 p-3 text-fg shadow-lg backdrop-blur-md"
>
