<script lang="ts">
	...
	let error = $state('');
	const chooser = popover({ lightDismiss: true });
	// Mounted on the first press, since a season lists one per episode. Kept
	// after, so the exit fades.
	...
	async function toggle() {
		...
		used = true;
		await tick();
		if (menu) void chooser.toggle(trigger, menu);
	}
...
{#if used}
	...
	>
		{#if choosingPause}
			<!-- No back button, a dismissed menu reopens on the actions. -->
